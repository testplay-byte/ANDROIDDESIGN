...
- **Custom scheme (recommended, simplest):** `<applicationId>://anilist-auth` — e.g. `com.confused.animetracker://anilist-auth`. Register this exact string in the AniList dev console and declare an `<intent-filter>` on a small `AnilistAuthActivity`:
  ```xml
  <activity android:name=".auth.AnilistAuthActivity"
      ...
      <intent-filter>
          ...
          <data android:scheme="com.confused.animetracker" android:host="anilist-auth"/>
      </intent-filter>
  </activity>
  ```
