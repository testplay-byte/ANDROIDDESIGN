...
> Critical review of the Phase 0 planning package for the ONLYLIST project.
...
12. **Repo structure matches CORE_RULES §4.** Single wrapper folder `ONLYLIST/` at repo root, `.github/workflows/` at repo root (GitHub constraint), zones inside the wrapper folder. 23 files committed in `3106091`, all intentional planning artifacts.
...
**What's wrong:**
- §0 table: "Wrapper folder | `ONLYLIST/` (single folder at repo root) | **proposed — awaiting confirmation**"
- §4 text: "The wrapper folder's name is the project name (`ONLYLIST/`)" — stated as fact, no "proposed" qualifier.
- `README.md` line 7: "All project content lives inside the `ONLYLIST/` wrapper folder." — stated as fact.

Per Q-003, the user confirmed the wrapper-folder rename but the new name "ONLYLIST" is still proposed. The documents disagree on whether the name is locked.

**Suggested fix:** Either:
- (a) Change §0 status to "confirmed (user approved wrapper rename; ONLYLIST matches repo name `testplay-byte/ONLYLIST` — pending only the formal Q-003 confirmation)"; OR
- (b) Add a footnote to §4 + README: "Wrapper folder name `ONLYLIST/` is tentative pending Q-003 confirmation."

Recommended: (a) — the user already approved the rename, and `ONLYLIST` matches the repo name (which IS confirmed).
...
6. **OAuth custom-scheme interception** — another app registering `com.confused.onlylist://` could capture the JWT.
...
| `anikuta` (lowercase, as a tag/package fragment) | CORE_RULES §20 (filtered logging — "Tags renamed `Anikuta:` → `Only-List:`" in Appendix A only) — actual rules use `Only-List:` | ✅ Renamed consistently. NOT a leak. |
